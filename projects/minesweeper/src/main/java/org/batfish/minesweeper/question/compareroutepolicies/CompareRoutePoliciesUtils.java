...
public final class CompareRoutePoliciesUtils {
  ...
  public Stream<Tuple<Result<BgpRoute>, Result<BgpRoute>>> getDifferencesStream2(NetworkSnapshot snapshot, boolean ignoreSubsets){
  ...
  private Stream<Tuple<Result<BgpRoute>, Result<BgpRoute>>> comparePolicyStanzasForNode(
    ...
    return policiesList.stream().flatMap(policy -> comparePolicyStanzas(policy, configAPs, snapshot, node, ignoreSubsets));
  }
  private Stream<Tuple<Result<BgpRoute>, Result<BgpRoute>>> comparePolicyStanzas(
    ...
    for (int i = 0; i < guards.size(); i++) {
      ...
      RoutingPolicy r1 = new RoutingPolicy(name + "_" + Integer.toString(i), owner);
      ...
      for (int j = i + 1; j < guards.size(); j++) {
        ...
        RoutingPolicy r2 = new RoutingPolicy(name + "_" + Integer.toString(j), owner);
        ...
        try{
          ...
          TransferBDD tBDD = new TransferBDD(factory, configAPs, r1);
          TransferBDD otherTBDD = new TransferBDD(factory, configAPs, r2);

          List<TransferReturn> paths = computePaths(tBDD);
          List<TransferReturn> otherPaths = computePaths(otherTBDD);

          BDD r1_accept_bdd = paths.get(0).getAccepted()? paths.get(0).getSecond(): paths.get(1).getSecond();
          BDD r2_accept_bdd = otherPaths.get(0).getAccepted()? otherPaths.get(0).getSecond(): otherPaths.get(1).getSecond();
          BDD r1_accept_bdd_complement = r1_accept_bdd.not();
          BDD r2_accept_bdd_complement = r2_accept_bdd.not();

          if(ignoreSubsets && (r1_accept_bdd_complement.or(r2_accept_bdd).isOne() || r2_accept_bdd_complement.or(r1_accept_bdd).isOne())) continue;
          else{
