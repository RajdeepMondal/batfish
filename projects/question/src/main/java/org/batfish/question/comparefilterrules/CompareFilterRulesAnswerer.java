...
public class CompareFilterRulesAnswerer extends Answerer {
    ...
    private static Stream<FilterDifference> compareFilterRules(
        ...
        for(int i=0;i<currentBdds.size();i++){
            BDD curMatchBDD = currentBdds.get(i).getMatchBdd(); // A
            BDD curMatchBDDComplement = curMatchBDD.not(); // not A

            for(int j=i+1;j<currentBdds.size();j++){
                if(takeDifferentActions(currentBdds.get(i), currentBdds.get(j))){

                    // We want to eliminate cases where one match condition is a subset of the other
                    // Remove these lines later on

                    BDD refMatchBDD = currentBdds.get(j).getMatchBdd(); // B
                    BDD refMatchBDDComplement = refMatchBDD.not(); // not B

                    if (ignoreSubsets) {
                        if (!curMatchBDDComplement.or(refMatchBDD).isOne() && // A -> B
                                !curMatchBDD.or(refMatchBDDComplement).isOne()) { // B -> A
